...
exports.deleteMedicalRecord = exports.updateMedicalRecord = exports.createMedicalRecord = exports.getMedicalRecordsByPatient = exports.getMedicalRecordById = exports.getMedicalRecords = void 0;
const database_1 = require("../config/database");
const errors_1 = require("../utils/errors");
const canAccessMedicalRecord = (record, user) => {
    if (!user)
        return false;
    // Admin ve todo
    if (user.role === 'admin') {
...
const getMedicalRecordById = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const result = await (0, database_1.query)(`SELECT m.*, p.id as patient_id, p.user_id as patient_user_id,
              p.first_name || ' ' || p.last_name as patient_name,
              d.id as doctor_id, d.user_id as doctor_user_id,
              u.first_name || ' ' || u.last_name as doctor_name,
              d.specialization
       ...
       WHERE m.id = $1 AND m.deleted_at IS NULL`, [id]);
        if (result.rows.length === 0) {
            throw new errors_1.AppError(404, 'Medical record not found');
        }
...
const getMedicalRecordsByPatient = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT m.id, m.record_number, m.visit_date, m.diagnosis, m.symptoms,
       ...
       ORDER BY m.visit_date DESC`, [patientId]);
        res.status(200).json({
            success: true,
            data: result.rows,
            total: result.rows.length,
...
const createMedicalRecord = async (req, res) => {
    try {
        ...
        const doctorCheck = await (0, database_1.query)(`SELECT id, user_id FROM doctors WHERE id = $1 AND deleted_at IS NULL`, [doctor_id]);
        ...
        const result = await (0, database_1.query)(`INSERT INTO medical_records (
...
const updateMedicalRecord = async (req, res) => {
    try {
        ...
        const currentRecord = await (0, database_1.query)(`SELECT m.*, d.user_id as doctor_user_id FROM medical_records m
       JOIN doctors d ON m.doctor_id = d.id
       WHERE m.id = $1 AND m.deleted_at IS NULL`, [id]);
        ...
        const result = await (0, database_1.query)(`UPDATE medical_records SET 
       ...
       RETURNING *`, [diagnosis, symptoms, treatment_plan, doctor_notes, vital_signs ? JSON.stringify(vital_signs) : null, id]);
        res.status(200).json({
            success: true,
...
const deleteMedicalRecord = async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`UPDATE medical_records SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);
        res.status(200).json({
            success: true,
...
exports.default = {
    getMedicalRecords: exports.getMedicalRecords,
    getMedicalRecordById: exports.getMedicalRecordById,
    getMedicalRecordsByPatient: exports.getMedicalRecordsByPatient,
    createMedicalRecord: exports.createMedicalRecord,
    updateMedicalRecord: exports.updateMedicalRecord,
    deleteMedicalRecord: exports.deleteMedicalRecord,
};
