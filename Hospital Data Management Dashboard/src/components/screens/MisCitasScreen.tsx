...
export function MisCitasScreen() {
  ...
  const loadAppointments = async () => {
    ...
    try {
      ...
      const appointments = await appointmentsApi.byPatient(patient.id);
      setCitas(appointments.map(mapAppointment));
    } catch (error) {
  ...
  const handleReschedule = async () => {
    ...
    try {
      await appointmentsApi.update(selectedCita.id, {
        appointment_date: newFecha,
        appointment_time: toApiTime(newHora),
        status: 'pending',
      });
      ...
      // Detectar si es error de conflicto
      if (errorMsg.toLowerCase().includes('already has an appointment') || errorMsg.toLowerCase().includes('409')) {
        toast.error('No disponible: Ya existe una cita a esa hora');
      } else {
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-sm hidden md:block">
        <CardContent className="p-0">
          {loading ? (
            <div className="p-8 text-center text-sm text-[#616161]">Cargando citas...</div>
          ) : loadError ? (
            <div className="p-8 text-center text-sm text-[#E53935]">{loadError}</div>
          ) : (
